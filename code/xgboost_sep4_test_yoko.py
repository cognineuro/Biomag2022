...
x_train_yoko = x_train[siteA,:]
y_train_yoko = y_train[siteA]
...
x_test_yoko = x_test[np.squeeze(np.where(label_test ==0)),:]
...
finetune_result = load('/raid/xuyierjing/biomag_competition/training/script_SMH/xgboost/XGB_finetune_sep4_LZCHil3_yoko.npy',allow_pickle=True)
finetune_result[:,1].sort()
yoko_param = finetune_result[2,5]
yoko_param2 = finetune_result[8,5]
yoko_param3 = finetune_result[9,5]

model = XGBClassifier(**yoko_param, random_state = 20)
model.fit(x_train_yoko, y_train_yoko, sample_weight=classes_weights) #    
y_test_pred = model.predict(x_test_yoko)

model2 = XGBClassifier(**yoko_param2, random_state = 20)
model2.fit(x_train_yoko, y_train_yoko, sample_weight=classes_weights)
y_test_pred2 = model2.predict(x_test_yoko)

model3 = XGBClassifier(**yoko_param3, random_state = 20)
model3.fit(x_train_yoko, y_train_yoko, sample_weight=classes_weights)
y_test_pred3 = model3.predict(x_test_yoko)

results = list([y_test_pred, y_test_pred2, y_test_pred3])
np.save('xgboost_sep4_test_yoko_result_balanced', results)
